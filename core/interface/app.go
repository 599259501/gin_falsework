...
func (app *App)refreshSession(){
	...
	sessionId := app.GetSession().GetSessionId()
	fmt.Println("获取到的sessionId=", sessionId)
	app.context.SetCookie(sessionName, sessionId,liftTime,"/","*",false,false)
}
...
func (app *App)GetSession()*service.Session {
	...
	app.session = service.NewSession(app.context)
